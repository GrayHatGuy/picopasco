...
//pid
#include <PID_v1.h> 
double PV, PIDout,SP = 20;
double Kp=2, Ki=5, Kd=1;
PID myPID(&PV, &PIDout, &SP, Kp, Ki, Kd, DIRECT);
int WindowSize = 5000;
...
#define PIN_PV 0 //#$ redefine global array
#define RELAY_PIN 6 //#$ redefine global array

const int mixIntSP = 3000;                    //while wait auto mix interval [ms]
...
int tmixSP[n] = {500,500,500,500};  //mix time for pump on [ms] 
...
void blink(long led_int) { 
  pinMode(ledPin, PIDout);
  pinMode(ledPin, PIDout);
  unsigned long currentMillis = millis(); 
  if ((currentMillis - previousMillis >= led_int) and (led_int > 0) ) {   //led_int PV in ms -1 toggle rn
...
// #$ if pv within (SP + tolSP) or (SP - tolSP) then set pidstatus = 1 complete else set to pidstatus = 0 main do while pid() if pidstatus =0
  windowStartTime = millis();  //#$ add prior to function call 

  myPID.SetOutputLimits(0, WindowSize); 
  myPID.SetMode(AUTOMATIC);
  PV = analogRead(TdsSensorPin); //read tds
  myPID.Compute();
  ...
  if (PIDout < millis() - windowStartTime) {
    digitalWrite(RELAY_PIN, HIGH); //#$ add a flip flop water and sum(nutrients)
  } else { 
    digitalWrite(RELAY_PIN, LOW);
  }

  //#$
  // switch case sensor only tds/ec rn
  // calculate return pid correction 
  // add algorithm for sum(n) nutrients dispensed at mix proportion vs counter water for pid.

  return PV;  //add status tolerance flag PV-SP actual vs sp
