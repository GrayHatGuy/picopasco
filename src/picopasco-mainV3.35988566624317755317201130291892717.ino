...
const int mixIntSP = 9876;                     //while wait auto mix interval [ms]
const int n =4;                               //number of pump relays or mixes
int ledPin = LED_BUILTIN;                    //led pin
int t_loop = 0;                             //while wait interval for loop()
...
int tmixSP[n] = {500,1000,3000,5000}; //mix time for pump on [ms] 
...
void loop() { 
  while ((millis()-looplast) > t_loop) {      //loop while wait 
    if (millis() > mixIntPV){                 //Set mix flag at interval
      mix = 1; 
      Serial.println("****> trigger mix = 1 - starting mix now! <****"); 
    } 
...
void sensors() { 
  ...
  printStr = String(millis())+"> t," + temperature + ",rh," + humidity; u16 tvoc_ppb, co2_eq_ppm; 
  err = sgp_measure_iaq_blocking_read(&tvoc_ppb, &co2_eq_ppm);      //read sgp30
  if (err == STATUS_OK) { 
    printStr = printStr + ",voc," + tvoc_ppb + ",co2," + co2_eq_ppm; 
  } 
  averageVoltage =  analogRead(TdsSensorPin) * (float)VREF/ 1024.0; float compensationCoefficient=1.0+0.02*(temperaturetds-25.0); //TDS calculations
  float compensationVoltage=averageVoltage/compensationCoefficient; 
  tdsValue=(133.42*compensationVoltage*compensationVoltage*compensationVoltage - 255.86*compensationVoltage*compensationVoltage + 857.39*compensationVoltage)*0.5; 
  sensorValue = analogRead(moisturePin); //read moisture sensore 
  printStr = printStr + ",ec," + compensationVoltage + ",tds," + tdsValue + ",h20," + sensorValue ; 
  String prntrelay =""; 
  for (int k=0; k<n;k++){ 
    prntrelay = prntrelay + ",S" + (k+1) + "," +String(tmixSP[k]) ; 
  }
  if (mixIntPV > millis()){
      Serial.println(String(printStr+prntrelay+"," + String(mixIntPV - millis()))+",mix="+mix); 
  } else {
    Serial.println(String(printStr+prntrelay)+",mix="+mix); 
  }
} 
...
void stiritup(){ 
  for (int m =0; m<n; m++){ 
    ...
    Serial.println ("#"+String(m+1)+" ON! for "+tmixSP[m]+" at "+millis()+" ends at "+pumpon);
      int cnt = 1;
    while (pumpon >= millis()) {                  //while pumping print countdown to end at fixed interval
      blink(-1);                                  //blink toggle on demand
      Serial.println("#"+String(m+1)+" "+(pumpon - millis())+"[ms]  of pump time remaining " + (mixprint-cnt) + "/" + mixprint ); cnt = cnt +1;
    ...
    Serial.println(String ("#" + String(m+1)+ " done! - " + millis())+"ms"); sensors();
  } 
  mix = 0; 
  mixIntPV =  mixIntSP + millis(); 
  Serial.println("***> " + String(mixcnt)+ " mixes since last boot.. mix=" + String(mix) + " <*** next mix at " + String(mixIntPV)); mixcnt = mixcnt +1;
}